...
public class DriverFirstActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        listView1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position,
                                    long id) {
                System.out.print(weatherList.get(position).R_id);
                Toast.makeText(getBaseContext(), String.valueOf(position), Toast.LENGTH_LONG).show();
                Intent intent = new Intent(DriverFirstActivity.this,PickupActivity.class);
    ...
    public void RequestList() {
        ...
        String[] columns = new String[] { status,email};
        // parameters
        Cursor res = db.rawQuery("Select * FROM Request,Reservation Where Request.R_id = Reservation.R_id AND R_status_id = ? AND Reservation.User_id=?", columns);
        int x = res.getCount();
    ...
    /*public void RequestListForDriver() {
        ...
        // parameters
        Cursor res = db.rawQuery("SELECT R_id FROM Reservation WHERE User_id=?", columns);
        ...
        while (res.moveToNext()) {// will go in this loop if there is a row with the given credentials
            *//*Weather entry = new Weather(res.getInt(res.getColumnIndex("R_id")), res.getString(res.getColumnIndex("Pick_up_location")), res.getString(res.getColumnIndex("Destination")));
            ...
            weatherList.add(entry);*//*
            RequestList(res.getInt(res.getColumnIndex("R_id")));
        }
*/
